...
{
    public static class GeradorDeMunicipios
    {
        #region Municipios
        ...
        public static void GerarMunicipioEUf(out string municipio, out string uf)
        {
            string nome = _nomes[_random.Next(0, _nomes.Length)];
            string[] partes = nome.Split(',');
            if (partes.Length != 2)
            {
                throw new ApplicationException(String.Format("O nome de município é inválido: '{0}'.", nome));
            }
            municipio = partes[0];
            uf = partes[1];
        }
